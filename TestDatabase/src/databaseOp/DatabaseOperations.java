...
public class DatabaseOperations {
	...
	{
		// TODO Auto-generated method stub
		System.out.print(email+ " From GetResourceId Method");
		Resource res=new Resource();
		try {
	        preparedStatement = this.connection.prepareStatement("select * from resource where email=?");
	        System.out.println(" prepared "+preparedStatement.toString());
...
//	}
	...
	public JSONArray generateJSON3() {

		String sql = "select fname,lname, department, billRate, costRate from capstone.resource;";
		JSONArray list = new JSONArray();
		ArrayList arrTest = null;
		...
		try {
			...
			while (rs.next()) {
				...
				arrTest = new ArrayList();
				if (i == 0) {
					arrTest.add("First Name");
					arrTest.add("Last Name");
					arrTest.add("Department");
					arrTest.add("Bill Rate");
					arrTest.add("Cost Rate");
					i++;
				} else {
					arrTest.add(rs.getString("fname"));
					arrTest.add(rs.getString("lname"));
					arrTest.add(rs.getString("department"));
					arrTest.add(rs.getDouble("billRate"));
					arrTest.add(rs.getDouble("costRate"));
	...
	public ResultSet displaySingleProject(String pname) throws SQLException {
		Connection c = Database.getConnection();
    	Statement st;
		
		//String sql1 = "select project.code, project.name, project.sdat, project.edate,project.budget,resource.fname, resource.lname, resource.billRate FROM project";
		 //String sql="select project.name,project.code,project.invoice,project.sdate,project.budget from capstone.project where name='"+pname+"'";
		 String sql= "select distinct p.name, p.code,p.invoice,p.sdate,p.budget, f.project, f.fname,f.hours, r.costRate, r.billRate, r.department from capstone.project p, capstone.filedata f,capstone.resource r where name='"+pname+"' AND f.project = p.name AND f.fname = r.fname";
		 //System.out.println(sql);
		 st = (Statement) c.createStatement();
		 ResultSet res =st.executeQuery(sql);
		// System.out.println(res);
...
public ResultSet displayAllProjects() throws SQLException
	...
	Statement st;
	//String sql1 = "select SUM(project.budget)FROM capstone.project";
	 //String sql= "select p.name,p.code,p.invoice,p.sdate,p.budget from capstone.project p";
	String sql= "select distinct p.code,p.invoice,p.sdate,p.budget, f.project, f.fname,f.hours, r.costRate from capstone.project p, capstone.filedata f,capstone.resource r where f.project = p.name AND f.fname = r.fname";
	 System.out.println(sql);
	 st = (Statement) c.createStatement();
	 ResultSet res =st.executeQuery(sql);
	 System.out.println(res);
...
public int editResources(int id,String fname,String lname,String email,String department,String billRate, String costRate,String permission)  {
	// TODO Auto-generated method stub
	...
	try {
		...
		Connection c = Database.getConnection();
    	Statement st = (Statement) c.createStatement();
		String sql ="UPDATE resource SET fname='"+fname+"', lname='"+lname+"', email='"+email+"', department='"+department
				+"', billRate='"+billRate+"',costRate='"+costRate+"', permission='"+permission+"' WHERE id="+id+"";
		System.out.println(sql); 
		res =st.executeUpdate(sql);
		c.close();
...
public int getResourceId(String email){
	// TODO Auto-generated method stub
			System.out.println("From getResourceId method--"+email);
			int id=0;
			Connection c = Database.getConnection();
	    	Statement st;
			try {
				st = (Statement) c.createStatement();
				String sql="SELECT id FROM resource WHERE email='"+email+"'";
				System.out.println(sql);
		    	ResultSet res =st.executeQuery(sql);
		    	while(res.next())
		    	{
		    		id=res.getInt("id");
		    		System.out.println(id+" from get resource id method");
		    	}
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
